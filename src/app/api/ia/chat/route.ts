...
export async function POST(request: Request) {
    ...
    if (!process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_API_KEY === 'sk-ant-your-api-key') {
        console.error('CRITICAL: ANTHROPIC_API_KEY is not configured or is using the placeholder.');
        return NextResponse.json(
            { message: 'Error de configuración: La clave de IA (ANTHROPIC_API_KEY) no está configurada correctamente en el servidor. Por favor, asegúrese de agregar una clave válida de Anthropic en las variables de entorno.' },
            { status: 500 }
        );
    }
    ...
    `;
        ...
        // Simple extraction logic (can be improved with structured output from AI)
        ...
        return NextResponse.json(
            { message: 'Hubo un error al procesar su mensaje con la IA. Por favor intente de nuevo.' },
            { status: 500 }
        );
    }
}
